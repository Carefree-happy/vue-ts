        ...
        return () => {
            ...
            if (isMultiType) {
                ...
                return schema.items!.map((s: Schema, index: number) => (
                    <SchemaItem schema={s} key={index} rootSchema={rootSchema} onChange={(v: any) => handleArrayItemChange(v, index)} value={arr[index]} />
                ))
            } else if (!isSelect) {
                // 不是单类型的渲染
                const arr = Array.isArray(value) ? value : []
                return arr.map((v: any, index: number) => {
                    return (
                        <ArrayItemWrapper index={index} onAdd={handleAdd} onDelete={handleDelete} onUp={handleUp} onDown={handleDown}>
                            <SchemaItem schema={schema.items as Schema} key={index} rootSchema={rootSchema} onChange={(v: any) => handleArrayItemChange(v, index)} value={v} />
                        </ArrayItemWrapper>
                    )
                })
            } else {
                ...
                return <SelectionWidget options={options || []} value={props.value} onChange={props.onChange} />
            }
